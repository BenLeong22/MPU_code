...
$u_sql ="SELECT * FROM user WHERE u_id = ".$u_id;
			...
			<div class="row"> 
				<div class="col-md-8">
                    ...
                    Status:     <?php     if($row["status"] ==1){
                                    ...
                                    echo '<td>Shipped Date: '.$row["develiveryDate"].'</td>';
                                }
       ...
       $pp_row =mysqli_fetch_assoc($pp_result);  
		...
		<table class="table table-hover">
			<tbody>
                ...
                <td style="float:left; "> Subtotal <br> MOP$'.$row["pp_subtotal"].' </td>';
        
            $pp1_sql = "SELECT * FROM purchaseorder WHERE PNo = ".$PNo;
            $pp1_result = mysqli_query($conn,$pp1_sql);
            $pp1_row = mysqli_fetch_assoc($pp1_result);
            if ($pp1_row["status"] == 2){
			echo '<td width="250"> <a href="comment.php?p_id='.$row["p_id"].'&PNo='.$PNo.'" class="btn btn-outline-primary">Comment</a>  ';
			}
